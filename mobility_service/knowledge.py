...
운영 서비스다. 현재 핵심 기능은 여러 보내는 사람의 픽업지에서 물품을 모아 여러
받는 사람에게 배송하는 스마트 딜리버리다. 기존 묶음퀵을 사용자에게 더 이해하기
쉬운 이름으로 바꾼 것이다. 받는 사람마다 보내는 사람을 연결하고, 개별 배송 합계와
하나의 경로로 묶은 가격을 비교한 뒤 사용자가 추천
경로와 최종 견적에 동의하면 하나의 주문으로 접수한다.

주소는 특정 도시로 제한하지 않는다. 카카오 주소 검색과 퀵 배송 API가 처리할 수
있는 국내 주소라면 서울, 경기뿐 아니라 대전 등 다른 지역도 입력할 수 있다.
    ...
    def fallback_answer(results: list[KnowledgeSearchResult]) -> str:
        if not results:
            return (
                ...
                "스마트 딜리버리 견적, 접수, 주문 조회, 취소 또는 Sandbox 이용 방법처럼 "
                "조금 더 구체적으로 물어봐 주세요."
            )
