...
class DeliveryAgent:
    ...
    async def _bundle_flow(self, state: AgentState) -> AgentState:
        ...
        try:
            result = await bundle_quote(self._client, self._geocoder, pickup, dropoffs)
    ...
    async def _pool_flow(self, state: AgentState) -> AgentState:
        ...
        candidates = [
            r for r in self._pools.list_open(exclude_session=state["session_id"])
            if is_compatible(r, my_request)
        ]
        if not candidates:
        ...
        other = candidates[0]
        try:
            quote = await pool_quote(self._client, [other, my_request])
        except (ValueError, KakaoApiError) as exc:
            return {
