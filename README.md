...
이 프로젝트의 핵심은 단순 RAG 챗봇이 아니라 **Kakao Mobility API를 사용해 배송
견적, 주문 생성, 상태 조회, 취소 흐름을 직접 연결한 개인 프로젝트**입니다. AI와
RAG는 서비스 기능을 더 편하게 쓰기 위한 보조 입구이고, 중심 기능은 모빌리티 API를
안전한 업무 흐름으로 감싼 배송 운영 서비스입니다.

일반적인 문서 Q&A 챗봇은 질문에 답한 뒤 끝나지만, MOVB는 질문의 종류에 따라
실제 Kakao Mobility Sandbox 기능으로 이어집니다.
...
- 기존 퀵 접수 폼에서 사람을 추가하면 스마트 딜리버리로 자동 전환 → 최신 견적 재검증, 동의 후 주문 접수
...
- 별도 모드 선택 없이 사람이 추가되면 일반 퀵에서 스마트 딜리버리로 자동 전환
- 받는 사람마다 어떤 보내는 사람의 물품인지 연결
- 모든 픽업을 마친 뒤 배송을 시작하는 안전한 경로 제약
...
- 기존 `/bundle` 주소는 홈페이지의 통합 퀵 접수 폼으로 자동 이동
...
2026-07-28 기준 현재 작업은 **Kakao Mobility API를 활용한 개인 모빌리티 서비스**로
설명할 수 있게 정리했습니다. 최근 변경은 스마트 딜리버리를 별도 페이지가 아니라
홈 화면의 기존 퀵 접수 흐름에 통합하는 작업입니다.

| 단계 | 작업 내용 | 상태 |
|---|---|---|
| API 연동 | Kakao Mobility Sandbox의 견적 조회, 주문 생성, 주문 상태 조회, 취소, Step 조회 흐름 연결 | 완료 |
| 안전 처리 | 사용자 동의 전 주문 생성 금지, 주문 직전 최신 견적 재검증, `Idempotency-Key` 중복 접수 방지 | 완료 |
| 경로 계산 | Kakao 길찾기 API와 주소 검색을 사용해 실도로 거리, ETA, 다중 경유지 순서 계산 | 완료 |
| 화면 통합 | `bundle.html` 별도 화면을 제거하고 `index.html`의 접수 폼에서 보내는 사람·받는 사람 추가 시 자동 전환 | 완료 |
| 라우팅 정리 | 기존 `/bundle`, `/smart-delivery/form` 접근은 `/#smartDelivery`로 이동시켜 이전 링크 호환 유지 | 완료 |
| 보조 지식 문서 | API 사용법과 스마트 딜리버리 조건을 챗봇이 설명할 수 있도록 Knowledge 문서 보강 | 완료 |
| 테스트 보강 | 홈 통합 화면, 자동 전환 문구, 스마트 딜리버리 견적·주문 API 연결, 이전 경로 리다이렉트 검증 | 완료 |
| 다음 작업 | LangChain 비교 실험과 Agent 의도·슬롯·도구 선택 평가 확대 | 진행 예정 |

이번 변경의 기준은 “AI 기술을 붙인 챗봇”보다 **Kakao Mobility API로 실제 배송 접수
프로세스를 어디까지 구현했는지**가 먼저 보이도록 만드는 것입니다. 사용자는 “일반
퀵”과 “스마트 딜리버리”를 별도 메뉴로 이해하지 않아도, 사람을 추가하는 자연스러운
행동만으로 묶음 배송 흐름에 들어갑니다.
