...
2026-07-28 기준 현재 작업은 **스마트 딜리버리를 별도 페이지가 아니라 홈 화면의
기존 퀵 접수 흐름에 통합하는 방향**으로 정리되었습니다.
...
| 지식 문서 갱신 | 홈페이지 통합 흐름, 묶음퀵 목적지 수, 수령인 연락처 필수 조건을 Knowledge RAG 문서에 반영 | 완료 |
| 테스트 보강 | 홈 통합 화면, 자동 전환 문구, 스마트 딜리버리 견적·주문 API 연결, 이전 경로 리다이렉트 검증 | 완료 |
| 다음 작업 | LangChain 비교 실험과 Agent 의도·슬롯·도구 선택 평가 확대 | 진행 예정 |

이번 변경의 기준은 사용자가 “일반 퀵”과 “스마트 딜리버리”를 별도 메뉴로 이해하지
않아도, 사람을 추가하는 자연스러운 행동만으로 묶음 배송 흐름에 들어가도록 만드는 것입니다.
